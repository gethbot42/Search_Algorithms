...
## For my breadth first searcher
class FrontierQueue(object):
    """Wrapper for a queue to add and pop"""

    def __init__(self):
        self.frontier_index = 0
        self.frontierQueue = queue.Queue()

    def empty(self):
        return self.frontierQueue.empty()

    def add(self, path):
        self.frontierQueue.put(path)

    def pop(self):
        return self.frontierQueue.get()

    def count(self):
        return self.frontierQueue.qsize()
...
class BreadthFirstSearcher(Searcher):
    """returns a breadth first searcher to a problem
	paths can be found by repeatedly calling search().
    """
...
## second attempt change 1 line
class BreadthFirstSearcherEnhanced(Searcher):
    def add_to_frontier(self, path):
...
## currently incomplete
class DepthLimitedSearcher(Searcher):

    def __init(self, depth_limit, problem):
        Searcher.__init__(problem)
    ...
    def search(self):
        ...
        while not self.empty_frontier():
            ...
            if self.problem.is_goal(path.end()):    # solution found
                ...
                self.display(3,"Neighbors are", neighs)
                for arc in reversed(neighs):
                    to_add = Path(path, arc)
                    if (len(list(to_add.nodes())) - 1) <= self.depth_limit:
                        self.add_to_frontier(Path(path,arc))
                self.display(3,"Frontier:",self.frontier)
        self.display(1,"No (more) solutions. Total of",
                     self.num_expanded,"paths expanded.")
